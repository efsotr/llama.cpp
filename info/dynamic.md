...
- Hexagon 后端的流水线包含独立的 Dynamic Quantizer 阶段，可通过 `GGML_HEXAGON_OPMASK` 的 `0x2` 位开启，仅量化或组合其它阶段；不使用动态量化时可跳过该阶段直接复用已有量化张量或只排队/计算。(docs/backend/hexagon/README.md#228-239, ggml/src/ggml-hexagon/htp/htp-msg.h#8-14)
